...
// an enum is mandatory, you need to pass this on to the statemachine constructor
enum class State {
    stand,
    crouch,
    prone
...
void BasicExample()
{
    ...
    sm.AddTransition(State::stand, State::crouch, [&buttonEntered]{ return buttonEntered[0] == 'c'; })
...
void StateCallBackExamples()
{
    ...
    sm.AddOnStart(State::stand , []{ std::cout << "Stood up!\n"; })
...
void UpdateMultiThreadedExample()
{
    ...
    // this will keep updating that statemachine while your main thread can still do other stuff
    std::thread thr{ [&sm] {
      while(true)
      {
        sm.Update();
